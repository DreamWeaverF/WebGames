...
export default {
    ...
    if (audios[id]) {
      const AddListener = () => {
        ...
        if (key === 'onCanplay') {
          audios[id][key]((e) => {
            // 兼容基础库获取属性异常的bug
            const { duration, buffered, referrerPolicy, volume } =
              audios[id]
            setTimeout(() => {
              moduleHelper.send('OnAudioCallback', JSON.stringify({
                callbackId: id,
  ...
  _JS_Sound_SetVolume(channelInstance, v) {
    ...
    try {
      ...
      let cur = soundVolumeHandler[channelInstance];
